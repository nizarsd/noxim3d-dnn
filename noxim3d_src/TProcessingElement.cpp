...
void TProcessingElement::rxProcess()
{
  ...
  else
  {
    if(req_rx.read()==1-current_level_rx)
    {
      ...
      {
        cout << sc_time_stamp().to_double()/1000 << ": ProcessingElement[" << local_id << "] RECEIVING " << flit_tmp << endl;
      }
      stats.power.Core();
...
void TProcessingElement::txProcess()
{
  ...
  else
  {
	...
	flit_tx->write(flit);  
	stats.power.Core();                   // Send the generated flit
...
TFlit TProcessingElement::nextFlit()
{
  ...
  flit.timestamp1  = sc_time_stamp().to_double()/1000;
  flit.sequence_no = packet.size - packet.flit_left;
  flit.packet_size = packet.size;
  flit.hop_no      = 0;
  flit.dw		   = 0;  // Initially set the downward times to zero
...
bool TProcessingElement::canShot(TPacket& packet)
{
  bool   shot;
  double threshold;
  float throt_pir=PEthrot_level.read()/100.0;  
  ...
  if (TGlobalParams::traffic_distribution != TRAFFIC_TABLE_BASED)
	...
	threshold = TGlobalParams::probability_of_retransmission;
     ...
     threshold*=throt_pir; // cosider router throttling when genrating packets <Nizar>
...
TPacket TProcessingElement::trafficRandom()
{
  ...
  int max_id = (TGlobalParams::mesh_dim_x * TGlobalParams::mesh_dim_y * TGlobalParams::mesh_dim_z )-1;
...
TPacket TProcessingElement::trafficTranspose1()
{
  ...
  dst.z = TGlobalParams::mesh_dim_z-1-src.z;
  fixRanges(src, dst);
...
TPacket TProcessingElement::trafficBitReversal()
{
  ...
  int nbits = (int)log2ceil((double)(TGlobalParams::mesh_dim_x*TGlobalParams::mesh_dim_y*TGlobalParams::mesh_dim_z));
  int dnode = 0;
  for (int i=0; i<nbits; i++)
    setBit(dnode, i, getBit(local_id, nbits-i-1));

    if (dnode >= TGlobalParams::mesh_dim_x*TGlobalParams::mesh_dim_y*TGlobalParams::mesh_dim_z)
  	{
	 int tp=rand()%4;
	 switch(tp)
	 {
	   case 0:
		...
		break;
	   case 1:
	   	dnode = TGlobalParams::mesh_dim_x-1;
		break;
	
	  case 2:
	   	dnode =TGlobalParams::mesh_dim_x*TGlobalParams::mesh_dim_y-TGlobalParams::mesh_dim_x;
		break;
	   case 3:
	   	dnode = TGlobalParams::mesh_dim_x*TGlobalParams::mesh_dim_y-1;
		break;
...
void TProcessingElement::fixRanges(const TCoord src, TCoord& dst)
{
  ...
  if(dst.z>=TGlobalParams::mesh_dim_z) dst.z=TGlobalParams::mesh_dim_z-1;
}
...
double TProcessingElement::getPower()
{
  return stats.power.getPowerCore();
}
