...
void TRouter::txProcess()
{
  if (reset.read())
      ...
      for(int i=0; i<DIRECTIONS+1; i++)
	{
	  ...
	  //Modification made by Ra'ed in 2010
       	  timeout_counter[i]=0;
	  aborted_flits_counter[i]=0;
          aborted_packets_counter[i]=0;
	  ch_abort_flag[i]=0;
	  if (i<DIRECTIONS)
	      {
	        tc_req_tx[i].write(0);
  ...
  else
    {
      ...
      bool d_flag[DIRECTIONS+1]={0,0,0,0,0};
      bool tc_ch_out[DIRECTIONS]={0,0,0,0};
      int  tc_ch_no=6;
	...
	{  
	  ...
	  if ( !buffer[i].IsEmpty() )
	      ...
	      if (flit.flit_type==FLIT_TYPE_HEAD) 
		{
		  ...
		  int o = route(route_data);
  		  if(TGlobalParams::verbose_mode > VERBOSE_OFF )
		  	 { cout << sc_time_stamp().to_double()/1000 
			       ...
			       << ", requested Output[" << o  << "], flit: " << flit << " Time= " << timeout_counter[o]<<"  free slots neighbor =" << free_slots_neighbor[o] << endl;
			  }
			  
		  if (o<DIRECTIONS)
		       tc_ch_out[o]= tc_req_rx[i].read() || tc_ch_out[o];
		  if (o==3 && tc_req_rx[i].read())
		  	tc_ch_no=i;

		  if (reservation_table.isAvailable(o) && (TGlobalParams::buffer_depth == free_slots_neighbor[o] || o== DIRECTION_LOCAL )) // not assigning output channel unless it is empty
		    {
		  ...
		  else  if (i != DIRECTION_LOCAL  && o!=DIRECTION_LOCAL && !d_flag[o])
		      {
		        timeout_counter[o]++;
			d_flag[o]=1;
			if (timeout_counter[o]>= TGlobalParams::timeout_threshold && TGlobalParams::deadlock_detection_flag && (TGlobalParams::channel_number1 == o 
									|| TGlobalParams::channel_number2 == o|| TGlobalParams::channel_number1 == 5))
			 		{
					  ch_abort_flag[i]=1;
					  timeout_counter[o]=0;
					}
      ...
      if(TGlobalParams::verbose_mode == 0 )
      {	
	cout << sc_time_stamp().to_double()/1000 << ": Router[" << local_id <<"]" ;
	for(int i=0; i<DIRECTIONS; i++)
	      cout  << "   ch["<< i << "] TC-ip=" << tc_req_rx[i].read() ;
	for(int i=0; i<DIRECTIONS; i++)   
	      cout  << "   ch["<< i << "] TC-op-req = " << tc_ch_out[i]; 
	...
	if (local_id ==2)
  	    {
	     deadlock_flag[3]=tc_ch_out[3];
	     tc_ch_out[3]=1;
	    }
	for(int i=0; i<DIRECTIONS; i++)
  	       tc_req_tx[i].write(tc_ch_out[i]);	
	  ...
	  if ( !buffer[i].IsEmpty() )
	    {
	      ...
	      if (ch_abort_flag[i]==1)
	       {     
	              buffer[i].Pop();
		      free_slots[i] = buffer[i].getCurrentFreeSlots();
		      aborted_flits_counter[i]++;
		      if (reservation_table.getOutputPort(i)!= NOT_RESERVED )
		      		reservation_table.release(reservation_table.getOutputPort(i));
				
		  if(TGlobalParams::verbose_mode > VERBOSE_OFF )    
		   { cout << " ********************************  flit aborted from channel " << i  << " ****************************************"<<endl  
		          << "Violation occurs.." << timeout_counter[i]<< ".."
		          << sc_time_stamp().to_double()/1000 
		          << ": Router[" << local_id 
		          << "], Input[" << i << "] (" << buffer[i].Size() << " flits)" 
		          << flit <<"  Aborted Flits = "<< aborted_flits_counter [i]<<" Aborted Packets="<< aborted_packets_counter [i]<<"  ";
			  
		    for(int k=0; k<DIRECTIONS+1; k++)
		      { int q = reservation_table.getOutputPort(k);
		        cout << k <<"-->" << q << " ";
		      }
		     cout << endl; 
		   }
		...
		if (flit.flit_type == FLIT_TYPE_TAIL) 
		   {
		    aborted_packets_counter[i]++;
		    ch_abort_flag[i]=0;

		    if(TGlobalParams::verbose_mode > VERBOSE_OFF )  
			cout << " $$$$$$$$$*************$$$$$$$$  a complete packet aborted from channel " << i << "$$$$$$$$$*********$$$$$$$$$$" << endl;
		   }
	        }  
	       else if (o != NOT_RESERVED )
		{
		...
		else if ((local_id==200 && deadlock_flag[3]==1 && tc_ch_no==i ) || (flit.flit_type != FLIT_TYPE_HEAD))
		{
