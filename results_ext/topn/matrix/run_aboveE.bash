...
ls $K/tables/*_aE*_k*.txt | while read f; do
  SET=$(basename $f | cut -d_ -f1); echo "$f ${SIM[$SET]}"
done | xargs -P 14 -n 2 bash -c 'source $K/.assoc4; run1 "$1" bl - "$2" 1' _ > $K/probe_aE.txt
echo "=== aE probe done $(date)"
python3 - <<'PY'
import collections
d=collections.defaultdict(dict)
for ln in open('results_ext/topn/matrix/probe_aE.txt'):
    p=ln.split()
    if p[3]=='NA': continue
    tag,k=p[0].rsplit('_k',1); d[tag][int(k)]=float(p[3])
with open('results_ext/topn/matrix/run_spec_aE.txt','w') as f:
    for tag in sorted(d):
...
done < $K/run_spec_aE.txt | xargs -P 14 -n 5 bash -c 'source $K/.assoc4; run1 "$1" "$2" "$3" "$4" "$5"' _ >> $K/res_aE.txt
echo "=== aE main done $(date)"; wc -l $K/res_aE.txt; grep -c NA $K/res_aE.txt
echo "AE DONE $(date)"
